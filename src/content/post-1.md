...
LAN 어댑터는 받은 데이터를 0101의 전기 신호로 변환하는데 비트 구분을 나타내는 클록신호와 함께 전송되며 앞에는 프리앰블 스타트 프레임딜리미터를 맨 끝에는 프레임 체크 시퀀스를 붙여서 데이터의 시작부분과 변환여부를 체크한다.
LAN어댑터에서 데이터를 수신했을 때, FCS체크 -> MAC 비교 -> 메모리 저장 -> 패킷 도착을 알려주는 인터럽트 생성 -> IP 받아서 확인 IP가 다르면 ICMP메세지를 사용하여 오류를 보내고 맞으면 디어셈블ㄹ링하여 TCP에 넘김 -> 소켓 찾음 -> 데이터 수신 버퍼에 저장 -> 앱이 가지러 오기를 기다림
